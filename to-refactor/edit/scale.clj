(defn set-block-size [world block-name original-scale
  ...
  (let [block (get-in world [:parts block-name])
        ...
        value (if (some neg? increase)
                -0.5
                0.5)
        part-rotation (transform/get-rotation (:transform block))
        rotation-transform (transform/create [0 0 0] part-rotation)
        offset (transform/apply rotation-transform
                                (vector/multiply scale-change value))
        new-center (vector/add original-center offset)
        new-transform (transform/create new-center part-rotation)]
    (-> world
...
(defn get-child-local-direction [world parent-name child-name]
  (let [parent (get-in world [:parts parent-name])
        ...
        rotation (transform/get-rotation-component relative-transform)]
    (transform/apply rotation [0 1 0])))
...
(defn move-children [world part-name scale-offset]
  (let [part (get-in world [:parts part-name])
        ...
        v (transform/apply (transform/get-inverse rotation) v)
        offset-transform (transform/create v [1 0 0 0])]
    (reduce (fn [w child-name]
              (let [old-transform (get (:saved-children-transforms world) child-name)]
                (if (in? child-name (:moving-children world))
                  (let [new-transform (transform/combine old-transform
                                                         offset-transform)]
                    (assoc-in w [:parts part-name :children child-name]
                              new-transform))
                  (let [part-transform (:transform part)
                        child-transform (transform/combine old-transform
                                                           (:saved-transform world))
                        new-transform (transform/remove child-transform part-transform)]
                    (assoc-in w [:parts part-name :children child-name]
                              new-transform)))))
            world
            (keys (:children part)))))
...
(defn scale-block-pressed [world event]
  (if-let [collision (get-part-collision world event)]
    (let [normal (get-collision-normal world collision)
          ...
          rotation-transform (transform/get-rotation-component (:transform part))
          v (transform/apply rotation-transform normal)
          scale (:scale part)
          center (transform/get-position (:transform part))
...
(defn scale-block-moved [world event]
  (if-let [block-name (:edited-part world)]
    (let [adjust-line (:adjust-line world)
          ...
          d (ag/line-line-closest-point adjust-line mouse-line)
...
(defn set-track-size [world track-name original-scale original-center height]
  (let [track (get-in world [:parts track-name])
        ...
        scale-change (vector/subtract new-scale original-scale)
        part-rotation (transform/get-rotation (:transform track))
        rotation-transform (transform/create [0 0 0] part-rotation)
...
(defn scale-track-pressed [world event]
  (if-let [{:keys [part-name point _]} (get-part-collision world event)]
    (let [part (get-in world [:parts part-name])
          ...
          center (transform/get-position transform)
          rotation-transform (transform/get-rotation-component transform)
          v (transform/apply rotation-transform [0 1 0])]
      (-> world
          (assoc-in [:edited-part] part-name)
...
(defn scale-cylinder-pressed [world event]
  (if-let [{:keys [part-name point index]}
    ...
    (let [part (get-in world [:parts part-name])
          inverse-transform (transform/get-inverse (:transform part))
          local-point (transform/apply inverse-transform point)
          ...
          rotation-transform (transform/get-rotation-component (:transform part))
          v (transform/apply rotation-transform local-normal)
          scale (:scale part)
          center (transform/get-position (:transform part))]
      (-> world
          (assoc-in [:edited-part] part-name)
...
(defn scale-display-pressed [world event]
  (if-let [collision (get-part-collision world event)]
    (let [part-name (:part-name collision)
          ...
          local-point (transform/apply inverse-transform (:point collision))]
